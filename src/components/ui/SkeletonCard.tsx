    ...
    <div className={`glass-card p-5 overflow-hidden relative ${className}`}>
      ...
      <div className="relative z-10 flex items-center justify-between mb-4">
        <div className="space-y-2">
          ...
          <div className="h-2 bg-white/[0.04] rounded-full w-32" />
        </div>
        <div className="w-10 h-10 bg-white/[0.06] rounded-xl" />
      </div>
      <div className="relative z-10 h-8 bg-white/[0.06] rounded-lg w-28 mb-2" />
      <div className="relative z-10 h-3 bg-white/[0.04] rounded-full w-20" />
    </div>
  )
}
...
export function SkeletonChart() {
  return (
    <div className="glass-card p-0 overflow-hidden relative min-h-[500px]">
      ...
      <div className="p-5 border-b border-white/[0.04] relative z-10 flex items-center justify-between">
        <div className="flex gap-2">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-8 bg-white/[0.06] rounded-lg w-12" />
          ))}
        </div>
      ...
      <div className="flex items-center justify-center h-full min-h-[400px] relative z-10">
        <LineChart size={48} className="text-white/[0.06] animate-pulse" />
      </div>
    </div>
  )
}
