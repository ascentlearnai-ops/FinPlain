    ...
    <span className="relative inline-flex">
      ...
      {open && (
        <div onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}
          className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-72 bg-[#1e2329] border border-white/[0.08] rounded-2xl shadow-xl p-4 animate-fade-up">
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-sm text-accent">{term}</span>
          ...
          <div className="absolute top-full left-1/2 -translate-x-1/2 w-2 h-2 bg-[#1e2329] border-r border-b border-white/[0.08] rotate-45 -mt-1" />
        </div>
      )}
    </span>
  )
